...
- [功能模块详细设计](#功能模块详细设计)
  ...
  - [3. 安全系统 (Security)](#3-安全系统-security)
  - [4. 上下文管理 (Context)](#4-上下文管理-context)
  - [5. Middleware Chain](#5-middleware-chain)
  - [6. 多智能体协作 (Multi-Agent)](#6-多智能体协作-multi-agent)
  - [7. 进化系统 (Evolution)](#7-进化系统-evolution)
  - [8. 专家智能体 (Expert Agent)](#8-专家智能体-expert-agent)
  - [9. Agent Teams](#9-agent-teams)
  - [10. 定时调度 (Scheduler)](#10-定时调度-scheduler)
  - [11. Gateway (L2)](#11-gateway-l2)
- [安全机制设计详解](#安全机制设计详解)
  - [安全设计哲学](#安全设计哲学)
  - [认证安全](#认证安全)
  - [授权安全](#授权安全)
...
| L3 | Harness | 企业级能力 | Memory / Skill / Security / Evolution / MultiAgent / Expert / Team / Sandbox |
...
│                                                                      │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │
│  │  Memory      │ │  Skill       │ │  Security    │ │  Context     │ │
│  │  四层记忆     │ │  指令+Plugin │ │  认证+审批    │ │  压缩+冲刷   │ │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │
│  │  MultiAgent  │ │  Evolution   │ │  Expert      │ │  Team        │ │
│  │  Sub+Bg任务  │ │  三Agent+GEPA│ │  Registry    │ │  TaskBoard   │ │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ │
│  ┌──────────────┐ ┌──────────────┐                                    │
│  │  Sandbox     │ │  Scheduler   │                                    │
│  │  Docker隔离  │ │  心跳+定时    │                                    │
...
| Skill | 类别 | 功能 |
|-------|------|------|
| file_manager | FILE_MANAGER | 文件读写搜索管理 |
| knowledge_search | KNOWLEDGE_SEARCH | 知识检索与问答 |
| report_generator | REPORT_GENERATOR | 报告模板生成 |
| schedule_manager | SCHEDULE_MANAGER | 日程安排管理 |
| notification | NOTIFICATION | 多渠道消息通知 |
| database_query | DATABASE_QUERY | 数据库查询分析 |
| data_analysis | DATA_ANALYSIS | 数据统计分析 |
...
| 角色 | 可用工具 | Skill 访问 | 审批上限 |
|------|----------|------------|----------|
| admin | 全部7工具 + spawn_subagent + 团队工具 | all | L3 |
| manager | 6工具(无command_exec) + spawn_subagent + 团队工具 | enterprise | L3 |
| operator | 5工具(只读) + send_notification | production | L3 |
| viewer | 4工具(最小集) | report | L2 |
...
#### 7.1 三 Agent 验证 (Three-Agent Verification)
...
#### 7.2 GEPA 进化优化 (Evolutionary Prompt Optimization)
...
- `scan_profiles()`：扫描 `agents/` 目录加载所有 YAML Profile
- `register()` / `get()` / `list_profiles()`：注册/查询/列表
- `load_soul_content()`：读取 SOUL.md 人格内容
...
|------|------|------|
| `/api/chat` | POST | 同步对话 |
| `/ws/chat` | WebSocket | 流式对话 + 会话持久化 |
| `/api/auth/token` | POST | JWT Token 创建 |
| `/api/skills` | GET | Skill 清单 |
| `/api/memory/{user_id}` | GET | 用户记忆文件 |
| `/api/sessions/{user_id}` | GET/POST | 会话列表/加载 |
| `/api/crons` | POST/GET/DELETE | 定时任务管理 |
| `/api/background` | POST/GET | 后台任务提交/查询 |
| `/api/agents` | GET | 专家智能体广场 |
| `/api/agents/{name}/chat` | POST | 专家智能体对话 |
| `/api/teams` | GET | 团队列表 |
| `/api/skills/verify` | POST | 三 Agent Skill 验证 |
| `/api/skills/optimize/{name}` | POST | GEPA Skill 优化 |
| `/api/evolution/auto` | POST | 触发自主进化 |
| `/api/plugins` | GET | Plugin 清单 |
| `/api/approvals/pending` | GET | L4 待审批列表 |
| `/api/approvals/{id}/approve` | POST | L4 批准 |
| `/api/approvals/{id}/reject` | POST | L4 拒绝 |
| `/api/dingtalk/callback` | POST | 钉钉机器人回调 |
| `/health` | GET | 健康检查 |
...
│  └─ create_expert_agent()
│     ├─ 加载 SOUL.md 人格文件 + Skill Plugin
│     ├─ system_prompt = 专属人格 + 技能指令
│     ├─ 工具集 = BASE_TOOLS（7 个基础工具）
...
| 创建函数 | `create_agent_for_user()` | `create_expert_agent()` |
| System Prompt | 通用企业助手 | SOUL.md 人格 + Skill Plugin 指令 |
| 工具集 | 按角色 BASE/ALL/CAPTAIN | 固定 BASE_TOOLS |
| 角色感知 | admin/manager 有队长/子智能体工具 | 不区分角色 |
...
- 标签页级：AdminPanel 内 `approvals` 标签仅 `admin`，`skills/crons/teams` 仅 `admin|manager`
