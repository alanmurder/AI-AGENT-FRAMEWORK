...
| L6 | Infrastructure | 存储运维 | PostgreSQL+pgvector / Redis / AgentScope Remote Sandbox / Docker Sandbox |
...
│    BASE_TOOLS(8) / ALL_TOOLS / CAPTAIN_TOOLS / MEMBER_TOOLS        │
└────────────────────────────────┬────────────────────────────────────┘
                                 │
┌────────────────────────────────┴────────────────────────────────────┐
│                    LLM PROVIDER (L5)                                  │
...
┌────────────────────────────────┴────────────────────────────────────┐
│                    INFRASTRUCTURE (L6)                                │
│    PostgreSQL+pgvector ─── Redis ─── Sandbox Runtime ─── File System │
└─────────────────────────────────────────────────────────────────────┘
```
...
Skill 是 Markdown 指令文档 (SKILL.md)，定义 Agent 在特定场景下的行为规范。不绑定专属工具，复用平台统一的 8 个基础工具（文件、Shell、Python、搜索、数据库、通知、记忆）。
...
|------|----------|----------|------------|----------|
| admin | 全部8个基础工具 + spawn_subagent + 团队工具 | 全部 (`"*"`) | all | L3 |
| manager | 6工具(无command_exec/python_exec) + spawn_subagent + 团队工具 | 按配置 (如 filesystem:read) | enterprise | L3 |
| operator | 5工具(只读) + send_notification | 无 | production | L3 |
| viewer | 4工具(最小集) | 无 | report | L2 |

**工具分层**：
- `BASE_TOOLS`(8)：file_read, file_write, command_exec, python_exec, web_search, query_database, send_notification, memory_manage
- `ALL_TOOLS`：BASE_TOOLS + spawn_subagent + submit_background_task (admin/manager 可见性仍受 RBAC 过滤)
- `CAPTAIN_TOOLS`：ALL_TOOLS + delegate_task, read_task_board, collect_results (队长)
- `MEMBER_TOOLS`：BASE_TOOLS + read_task_board (队员)
...
| **L2** | 白名单分类 | 安全命令放行，非白名单阻断 | command_exec 白名单(ls/cat/grep/python/git 等)、python_exec 需更高审批、query_database 仅允许 SELECT、file_write 禁止写系统目录(/etc/bin/usr 等) |
...
- **统一沙箱抽象**：`SandboxManager` 按 `user_id + session_id + agent_id` 管理会话，后端统一实现 `run_shell`、`run_python`、`read_file`、`write_file`、`list_files`。
- **可插拔后端**：生产默认 `agentscope_remote`，连接 AgentScope Runtime Sandbox；开发/离线可切 `local_docker`；`disabled` 仅用于本地调试。
- **覆盖范围扩大**：`file_read`、`file_write`、`command_exec`、`python_exec` 都进入沙箱工作区，不再只隔离命令执行。
- **Fail closed**：`sandbox_fail_closed=true` 时沙箱不可用直接返回安全错误，不回退宿主机执行。
- **路径边界**：所有相对路径解析到沙箱 workspace，拒绝绝对路径、`..` 逃逸、Windows 盘符路径；本地 Docker 后端额外用 `realpath` 防软链接越界。
- **镜像策略**：`SandboxImageManager` 仅服务于 `local_docker` 后端的镜像准备，不是生产主路径。
...
SecurityCheckMiddleware ──── L0-L4 审批链 (Wrap Tool)
  │
  ▼
SandboxMiddleware ────────── 文件/Shell/Python 路由到沙箱 (Wrap Tool)
  │
  ▼
...
└─ agent_id 为空 或 未注册
   └─ create_agent_for_user()
      ...
      │   ├─ admin/manager + team_enabled → CAPTAIN_TOOLS（再经 RBAC 过滤）
      │   ├─ admin/manager               → ALL_TOOLS（再经 RBAC 过滤）
      │   └─ operator/viewer             → BASE_TOOLS(8)（再经 RBAC 过滤）
...
4. ContextEditingMiddleware   → 删除低价值消息
5. PreFlushMiddleware         → 上下文接近上限时冲刷到中期记忆
6. ToolFilterMiddleware       → 根据 RBAC 过滤越权工具
7. SecurityCheckMiddleware    → L0→L1→L2→L3→L4 五级审批 (仅危险工具)
8. SandboxMiddleware          → 沙箱隔离文件/Shell/Python 工具
9. OutputValidationMiddleware → 校验输出格式
10. MemoryArchiveMiddleware   → 会话内容归档到长期记忆
```
...
| 控制项 | 实现 |
|--------|------|
| 工具隔离 | `file_read` / `file_write` / `command_exec` / `python_exec` 统一进入沙箱 |
| 后端隔离 | 生产默认 AgentScope Remote Sandbox，开发可用 Local Docker |
| 网络隔离 | 默认禁止网络访问 (`sandbox_network_default=deny`) |
| 超时控制 | 默认 30 秒超时，本地 Docker 后端超时后 kill 容器 |
| Fail closed | 沙箱不可用时危险工具直接失败，不回退宿主机 |
| 输出截断 | 本地 Docker stdout/stderr 最大 10000 字符 |
| 文件写入保护 | L2 审批禁止系统路径，沙箱层拒绝绝对路径、`..` 逃逸和软链接越界 |
...
| 配置项 | 默认值 | 环境变量 | 说明 |
|--------|--------|----------|------|
| `sandbox_enabled` | `True` | `AI_AGENT_SANDBOX_ENABLED` | 启用沙箱中间件 |
| `sandbox_backend` | `agentscope_remote` | `AI_AGENT_SANDBOX_BACKEND` | 沙箱后端：`agentscope_remote` / `local_docker` / `disabled` |
| `sandbox_fail_closed` | `True` | `AI_AGENT_SANDBOX_FAIL_CLOSED` | 沙箱不可用时是否阻断危险工具 |
| `sandbox_base_url` | `http://sandbox-runtime:8000` | `AI_AGENT_SANDBOX_BASE_URL` | AgentScope Runtime Sandbox 服务地址 |
| `sandbox_bearer_token` | `""` | `AI_AGENT_SANDBOX_BEARER_TOKEN` | 远程沙箱鉴权 token |
| `sandbox_network_default` | `deny` | `AI_AGENT_SANDBOX_NETWORK_DEFAULT` | 默认网络策略 |
| `sandbox_session_ttl_seconds` | `3600` | `AI_AGENT_SANDBOX_SESSION_TTL_SECONDS` | 沙箱会话 TTL |
| `sandbox_docker_image` | `ai-agent-sandbox:latest` | `AI_AGENT_SANDBOX_DOCKER_IMAGE` | local_docker 后端镜像 |
| `sandbox_timeout_seconds` | `30` | `AI_AGENT_SANDBOX_TIMEOUT_SECONDS` | Shell/Python 执行超时 |
| `sandbox_max_memory` | `256m` | `AI_AGENT_SANDBOX_MAX_MEMORY` | local_docker 内存限制 |
...
YAML 提供嵌套结构的配置，其中一部分映射到 `AgentConfig`，另一部分由具体模块直接读取：
...
| `sandbox` | 后端、fail closed、远程地址、网络策略、TTL、Docker 限额 | 映射到 `AgentConfig.sandbox_*` 字段 |
...
│   ├── sandbox/              # 沙箱执行
│   │   ├── manager.py        # SandboxManager (会话生命周期/路径策略/审计)
│   │   ├── backends.py       # AgentScope Remote / Local Docker / Disabled 后端
│   │   ├── types.py          # SandboxResult / SandboxError
│   │   ├── runner.py         # 旧版 SandboxRunner (兼容测试)
│   │   └── image.py          # SandboxImageManager (local_docker 镜像准备)
│   └── scheduler/            # 定时调度
...
│   ├── tools.py              # 8基础工具 + spawn_subagent + 后台/团队工具
│   └── __init__.py
│
├── agents/                   # 专家智能体定义
...
| 沙箱 | AgentScope Runtime Sandbox (生产默认) + Docker local backend |
| 定时 | APScheduler |
